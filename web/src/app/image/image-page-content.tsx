...
import {
  ...
  createPublicImageEditTasks,
  createPublicImageGenerationTasks,
  fetchAccounts,
  fetchImageTasks,
  fetchPublicImageQuota,
  fetchPublicImageTasks,
  type Account,
  type ImageTask,
} from "@/lib/api";
import {
  clearImageConversations,
  clearPublicImageConversations,
  deleteImageConversation,
  deletePublicImageConversation,
  getImageConversationStats,
  listImageConversations,
  listPublicImageConversations,
  renameImageConversation,
  renamePublicImageConversation,
  saveImageConversation,
  saveImageConversations,
  savePublicImageConversation,
  savePublicImageConversations,
...
const PUBLIC_ACTIVE_CONVERSATION_STORAGE_KEY = "chatgpt2api:public_image_active_conversation_id";
const PUBLIC_IMAGE_SIZE_STORAGE_KEY = "chatgpt2api:public_image_last_size";
const PUBLIC_IMAGE_COUNT_STORAGE_KEY = "chatgpt2api:public_image_last_count";

function clampImageCount(value: string, max = 100) {
  return String(Math.min(max, Math.max(1, Math.floor(Number(value) || 1))));
}
...
const privateConversationStore = {
  list: listImageConversations,
  save: saveImageConversation,
  saveMany: saveImageConversations,
  rename: renameImageConversation,
  delete: deleteImageConversation,
  clear: clearImageConversations,
};

const publicConversationStore = {
  list: listPublicImageConversations,
  save: savePublicImageConversation,
  saveMany: savePublicImageConversations,
  rename: renamePublicImageConversation,
  delete: deletePublicImageConversation,
  clear: clearPublicImageConversations,
};
...
async function syncConversationImageTasks(
  ...
  taskFetcher: (ids: string[]) => ReturnType<typeof fetchImageTasks>,
  saveMany: (conversations: ImageConversation[]) => Promise<void>,
) {
  ...
  try {
    taskList = await taskFetcher(taskIds);
  } catch {
    return items;
  }
...
async function recoverConversationHistory(
  ...
  return syncConversationImageTasks(normalized, taskFetcher, saveMany);
}


export function ImagePageContent({ isAdmin, publicMode = false }: { isAdmin: boolean; publicMode?: boolean }) {
  ...
  const maxImageCount = publicMode ? 6 : 100;
  const conversationStore = publicMode ? publicConversationStore : privateConversationStore;
  const taskFetcher = publicMode ? fetchPublicImageTasks : fetchImageTasks;
  const activeConversationStorageKey = publicMode ? PUBLIC_ACTIVE_CONVERSATION_STORAGE_KEY : ACTIVE_CONVERSATION_STORAGE_KEY;
  const imageSizeStorageKey = publicMode ? PUBLIC_IMAGE_SIZE_STORAGE_KEY : IMAGE_SIZE_STORAGE_KEY;
  const imageCountStorageKey = publicMode ? PUBLIC_IMAGE_COUNT_STORAGE_KEY : IMAGE_COUNT_STORAGE_KEY;
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        const storedCount = typeof window !== "undefined" ? window.localStorage.getItem(imageCountStorageKey) : null;
        setImageSize(storedSize || "");
        setImageCount(storedCount ? clampImageCount(storedCount, maxImageCount) : "1");

        const items = await conversationStore.list();
        const normalizedItems = await recoverConversationHistory(items, taskFetcher, conversationStore.saveMany);
        if (cancelled) {
          return;
        }
  ...
  }, [activeConversationStorageKey, conversationStore, imageCountStorageKey, imageSizeStorageKey, maxImageCount, taskFetcher]);
  ...
  const loadQuota = useCallback(async () => {
    if (!isAdmin) {
      ...
      try {
        const data = await fetchPublicImageQuota();
        setAvailableQuota(String(Math.max(0, Number(data.quota) || 0)));
      } catch {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && parsedCount > 0) {
      window.localStorage.setItem(imageCountStorageKey, String(parsedCount));
    }
  }, [imageCountStorageKey, parsedCount]);
  ...
  const updateConversation = useCallback(
    async (
      ...
      if (options.persist !== false) {
        await conversationStore.save(nextConversation);
      }
    },
    [conversationStore],
  );
  ...
  const runConversationQueue = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        const submitted = publicMode
          ? (
              activeTurn.mode === "edit"
                ? await createPublicImageEditTasks(pendingTaskIds, referenceFiles, activeTurn.prompt, activeTurn.model, activeTurn.size)
                : await createPublicImageGenerationTasks(pendingTaskIds, activeTurn.prompt, activeTurn.model, activeTurn.size)
            ).items
          : await Promise.all(
        ...
        while (true) {
          ...
          await sleep(2000);
          const taskList = await taskFetcher(loadingTaskIds);
          ...
          if (taskList.missing_ids.length > 0 && latestTurn) {
            ...
            const resubmitted = publicMode
              ? (
                  activeTurn.mode === "edit"
                    ? await createPublicImageEditTasks(missingTaskIds, referenceFiles, activeTurn.prompt, activeTurn.model, activeTurn.size)
                    : await createPublicImageGenerationTasks(missingTaskIds, activeTurn.prompt, activeTurn.model, activeTurn.size)
                ).items
              : await Promise.all(
    ...
    [loadQuota, publicMode, taskFetcher, updateConversation],
  );
  ...
  return (
    <>
      <section className={`mx-auto grid min-h-0 w-full max-w-[1380px] grid-cols-1 gap-2 overflow-hidden px-0 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] sm:gap-3 sm:px-3 sm:pb-6 lg:grid-cols-[240px_minmax(0,1fr)] ${publicMode ? "h-[calc(100dvh-2rem)] sm:h-[calc(100dvh-2.5rem)]" : "h-[calc(100dvh-6.5rem)] sm:h-[calc(100dvh-5.25rem)]"}`}>
